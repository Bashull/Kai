...
class LongContextPolicyTests(unittest.TestCase):
    ...
    def test_activation_offload_without_exact_transformers_identity_requires_canary(self):
        ...
        self.assertIn("transformers_activation_offload_identity", result.canaries)
    ...
    def test_million_token_profile_requires_allocator_guard(self):
        result = evaluate_long_context_profile({"target_tokens": 1_048_576, "native_context_tokens": 40_960, "position_extension_strategy": "yarn", "parallelism": "cp", "backend": "fsdp2", "accelerate": "1.14.0", "activation_offload": True, "transformers_source": "release", "transformers_version": "5.16.0", "expandable_segments": False})
        self.assertEqual(result.status, LongContextStatus.NEEDS_CANARY)
