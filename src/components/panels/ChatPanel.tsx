...
const ChatPanel: React.FC = () => {
  ...
  const {
    ...
    constitution, chi, auditChi, restoreChi, addDiaryEntry, addNotification
  } = useAppStore();
  ...
  const handleSend = async () => {
    ...
    const touchesCodebase = /(c[oó]digo|script|repo|archivo|patch|commit|merge|refactor|bug|fix|implementa|modifica)/i.test(userMessageContent);
    const touchesMemory = /(memoria|recuerda|guarda|absorbe|integra|n[uú]cleo|identidad|constituci[oó]n|chi)/i.test(userMessageContent);
    const destructive = /(borra|elimina|destruye|wipe|purga|rm -rf|sweep|absorbe todo)/i.test(userMessageContent);
