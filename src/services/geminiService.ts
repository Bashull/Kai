...
export const streamChat = async function* (
  ...
  if (functionCalls.length > 0) {
    ...
    if (options?.constitution && options?.chi) {
      const touchesCodebase = /(code|repo|file|patch|forge|studio|kernel|task|analysis|video)/i.test(toolNames);
      const touchesMemory = /(kernel|snapshot|diary|memory|chat)/i.test(toolNames);
      const destructive = /(delete|remove|wipe|purge|clear|reset)/i.test(toolNames);

      const preflight = runConstitutionalPreflight({
        prompt: `Ejecutar herramientas: ${toolNames}`,
      ...
      if (preflight.blocked) {
        yield {
          text:
            `\n\n**Preflight constitucional bloqueó la llamada de herramientas.**\n\n` +
            `${preflight.suggestedReply ?? 'No voy directo con esto.'}\n\n` +
      ...
      yield {
        text: `\n\n*Preflight OK para herramientas: ${toolNames}.*\n\n`
      };
    } else {
      yield {
        text: `\n\n*Herramientas detectadas: ${toolNames}.*\n\n`
      };
    }
...
interface GenerateVideoParams {
  ...
  config: { aspectRatio: '16:9' | '9:16'; resolution: '720p' | '1080p'; numberOfVideos: number; };
